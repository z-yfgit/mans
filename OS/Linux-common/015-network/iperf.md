...
| 选项 | 说明
| --- | ---
| -t              | 侦听新连接和接收流量的时间(以秒为单位)(默认未设置)
| -p              | 指定服务端要监听或者客户端要连接的端口
| -f [kmgKMG]     | 指定报告格式 <br>k == Kbits, K == KBytes<br>m == Mbits, M == MBytes<br>g == Gbits, G == GBytes
| -i              | 指定报告间隔
| -D              | 将服务器作为守护进程运行
...
```sh
iperf -s -p 7788
iperf -c 172.16.30.100 -p 7788 -t 1024 -i 1 -P 1 -M 1000 -m
